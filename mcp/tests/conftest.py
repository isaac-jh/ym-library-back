...
원본 프로덕션 ORM 과 컬럼명/타입이 일치하므로, SQL 호환되는 범위에서
같은 쿼리가 동작함을 확인할 수 있다.
"""
...
# `mcp/` 를 PYTHONPATH 에 추가해, 프로덕션 코드와 동일한 import 경로를 사용.
...
def _setup_schema_and_seed():
    ...
    from db import Base, BackupStatus, StorageCatalog, engine, session_scope
    ...
    with session_scope() as session:
        # write 가능하게 readonly 컨텍스트지만 commit 강제 호출.
