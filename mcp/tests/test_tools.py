...
def test_find_by_activity_name_korean() -> None:
    ...
    result = tools.find_video_backup_location(keyword="가족초청예배")
...
def test_find_by_alias_in_description() -> None:
    ...
    result = tools.find_video_backup_location(keyword="가초예")
...
def test_find_with_year_filter() -> None:
    """year 필터가 정상 동작한다."""
    result = tools.find_video_backup_location(keyword="가족초청예배", year=2025)
...
def test_find_empty_keyword() -> None:
    ...
    result = tools.find_video_backup_location(keyword="")
...
def test_search_by_description_hits_both_tables() -> None:
    ...
    result = tools.search_by_description(keyword="손을 들고 찬양")
    # 카탈로그: '회중이 손을 들고 찬양하는 장면 다수 포함'
    assert any(
        item["storage"] == "CLOUD" for item in result["catalog_matches"]
    )
    # 백업: '후반부 손 든 회중 컷' 은 '손을 들고 찬양' 과는 부분 일치하지 않을 수도 있어
    # 다른 케이스도 별도 검증한다.
...
def test_search_by_description_attaches_location_hint() -> None:
    ...
    result = tools.search_by_description(keyword="손 든 회중")
...
def test_list_recent_activities_limit() -> None:
    """limit 파라미터가 적용된다."""
    result = tools.list_recent_activities(limit=2)
    assert result["count"] == 2
